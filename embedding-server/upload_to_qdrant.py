...
# ====== 중복 컬럼 검사(CONTENTS 기준) ======
def is_duplicate_contents(contents: str) -> bool:
    scroll_body = {
        "filter": {
            "must": [
                {"key": "CONTENTS", "match": {"value": contents}}
            ]
        },
        "limit": 1
    }
    ...
    try:
        ...
        print(f"[ERROR] 중복 체크 실패 (CONTENTS={contents}): {e}")
        return False
...
for _, row in tqdm(df.iterrows(), total=len(df)):
    ...
    if is_duplicate_contents(contents):
        print(f"[SKIP] 중복된 컨텐츠 내용: {contents}")
        continue

    category = row.get("카테고리", "")
    text = f"{category} {contents}".strip()
    ...
    try:
        ...
        print(f"[ERROR] 임베딩 실패 (컨텐츠 내용={row.get('내용', '')}): {e}")
        continue
    ...
    try:
        ...
        print(f"[ERROR] Qdrant 저장 실패 (컨텐츠 내용={row.get('내용', '')})")
